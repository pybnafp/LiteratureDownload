...
# 当前激活查询: 15个 (总计约800篇种子文献)
#
# 历史注释: 以下查询已被暂时禁用，如需重新启用请检查git历史恢复
# - Dysarthria干预/治疗 (50篇) - 核心类
# - ASD言语/语言诊断/评估 (50篇) - 核心类
# - ASD言语/语言干预/治疗 (50篇) - 核心类
# - DLD诊断/评估 (50篇) - 核心类
# - 言语语言病理学指南 (20篇) - 重要类
# - 沟通障碍康复 (20篇) - 重要类
#
...
class BatchSpeechDisorderDownloader:
    ...
    def __init__(
        ...
        # 测试模式下不加载已有DOI记录，以加快测试速度
        if not self.test_mode:
            self._load_existing_dois()
        else:
            logger.info("【测试模式】未加载已有DOI记录，将尝试重新下载所有文献（不会覆盖已存在的PDF文件）")
    ...
    def batch_search_and_download(
        ...
        :param limit: 限制处理的文献总数（用于快速测试），None表示不限制
        """
        print("\n" + "=" * 80)
        print("开始批量搜索和下载言语障碍相关文献")
        if self.test_mode:
            print("【测试模式】强制重新下载所有文献（不跳过已下载的），用于快速测试")
        if limit:
            print(f"【限制模式】仅处理前 {limit} 篇文献")
        print("=" * 80)
        ...
        total_found = 0
        total_new_dois = 0
        total_processed = 0  # 总处理计数器（用于limit限制）
        ...
        for idx, search_config in enumerate(search_queries, 1):
            ...
            try:
                ...
                for article in articles:
                    ...
                    if skip_existing and not self.test_mode and (
                ...
                # 将新文献分成多个批次
                # 如果设置了limit限制，调整文献数量
                if limit and total_processed + len(new_articles) > limit:
                    new_articles = new_articles[:limit - total_processed]
                    if not new_articles:
                        print(f"\n已达到处理限制 {limit} 篇文献，停止处理")
                        break
                ...
                for batch_idx in range(total_batches):
                    ...
                    for article_idx, article in enumerate(current_batch, 1):
                        ...
                        normalized_doi = clean_doi(doi)
                        # 测试模式下不检查是否已下载，强制重新处理
                        if not self.test_mode and (
                            self.downloader.recorder.is_downloaded(doi) or self.downloader.recorder.is_failed(doi)
                        ):
                        ...
                        # 如果设置了limit且达到限制，停止处理
                        if limit and total_processed >= limit:
                            print(f"\n已达到处理限制 {limit} 篇文献，停止当前批次处理")
                            break
                    ...
                    # 如果还有下一个批次，显示提示信息
                    if batch_idx < total_batches - 1 and (not limit or total_processed < limit):
    ...
    def bfs_search_and_download(
        ...
        while queue and len(self.downloader.recorder.downloaded_dois) < target_absolute:
            ...
            for ref in refs:
                ...
                child_norm = clean_doi(child_doi)
...
if __name__ == "__main__":
    ...
    # Selenium：默认全程无头、不弹窗, 若必须在有界面环境调试，可在 .env 设置 SELENIUM_HEADLESS=false
    SELENIUM_HEADLESS = os.getenv("SELENIUM_HEADLESS", "false").lower()
    
    # 参考文献解析：是否使用通义 LLM 从 raw_citation 提取标题等（关闭则仅用 XML/正则，加快测试）
    USE_LLM_FOR_REFERENCES = True
    ...
    parser = argparse.ArgumentParser(
        ...
        # 测试模式（强制重新下载，不跳过已下载的，用于快速测试）
        python batch_speech_disorder_downloader.py --test --limit 10
    ...
    parser.add_argument(
        ...
        action='store_true',
        default='true',
        help='测试模式：强制重新下载所有文献，不跳过已下载的（用于快速测试下载流程）'
    )
    parser.add_argument(
        '--limit',
        type=int,
        default=20,
        metavar='N',
        help='限制处理的文献总数为N篇（用于快速验证流程），默认不限制'
    )
    ...
    if args.test:
        logger.info("【测试模式】已启用，将强制重新下载所有文献（不跳过已下载的）")
    if args.limit:
        logger.info(f"【限制模式】仅处理前 {args.limit} 篇文献")
